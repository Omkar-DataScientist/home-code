...
dbutils.widgets.dropdown("once", "True", ["True","False"], "Only Once")
...
# MAGIC %run ../../Includes/Classroom-Setup-4.03.2
...
def process_bronze(source, table_name, checkpoint, once=False, processing_time="5 seconds"):
    ...
    if once == True:
        data_stream_writer.trigger(once=True).table(table_name).awaitTermination()
    else:
        data_stream_writer.trigger(processingTime=processing_time).table(table_name)
...
def heart_rate_silver(source_table="bronze", once=False, processing_time="10 seconds"):
    ...
    if once == True:
        data_stream_writer.trigger(once=True).start().awaitTermination()
    else:
        data_stream_writer.trigger(processingTime=processing_time).start()
